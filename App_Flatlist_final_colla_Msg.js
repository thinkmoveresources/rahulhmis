...
const ExpandableComponent = ({ item, onClickFunction, listDataSource }) => {
  ...
  const changeText = (txt, item) => {
    console.log(CONTENT); 
    console.log("Padssed Item")
    console.log(item.category_name);    
    const newUpdateMsg = [...CONTENT];
    console.log("Original");
    console.log(newUpdateMsg);
    const cpt_subitm=newUpdateMsg[item.id].subcategory;
   const tmpfound = cpt_subitm.findIndex(
     (cpt_subitm) => cpt_subitm === item.id
   );
    console.log("Out put ");
    console.log(cpt_subitm);
    console.log(newUpdateMsg[item.id - 1].subcategory[item.id - 1].fieldtype);
      ...
      newUpdateMsg[item.id - 1].subcategory[item.id - 1] = Object.assign(
        newUpdateMsg[item.id - 1].subcategory[item.id - 1],
        {
          badMessage:
            tempmsg.errors +
            ": " +
            newUpdateMsg[item.id - 1].subcategory[
              item.id - 1
